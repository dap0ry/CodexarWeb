...
require(['vs/editor/editor.main'], function () {
    ...
    initTournamentBattle();
});

// --- Init ---
async function initTournamentBattle() {
    const token = localStorage.getItem('access_token');
    if (!token) { window.location.href = '/login'; return; }

    const params = new URLSearchParams(window.location.search);
    const matchId = params.get('id');
    if (!matchId) { window.location.href = '/torneos'; return; }

    try {
        const res = await fetch(`${API_BASE}/matchmaking/match/${matchId}`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        if (!res.ok) { window.location.href = '/torneos'; return; }
        ...
        loadTournamentBanner(tournamentId);
    ...
    document.getElementById('btnBackTournament').addEventListener('click', () => {
        const dest = tournamentId ? `/torneos?t=${encodeURIComponent(tournamentId)}` : '/torneos';
...
async function loadTournamentBanner(tId) {
    if (!tId) return;
    try {
        const token = localStorage.getItem('access_token');
        const res = await fetch(`${API_BASE}/tournaments/${tId}`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        if (!res.ok) return;
        const t = await res.json();
        const banner = document.getElementById('tournamentBanner');
        const text = document.getElementById('tournamentBannerText');
        text.textContent = `⚔ Torneo "${t.name || t.title || 'Torneo'}"`;
...
function endMatch(isWinner) {
    ...
    if (isWinner) {
        ...
        title.className = "win";
        desc.textContent = "¡Has avanzado de ronda!";
    } else {
        title.textContent = "DERROTA";
        title.className = "lose";
        document.getElementById('opProgressBar').style.width = '100%';
        desc.textContent = "Has sido eliminado del torneo.";
    }
