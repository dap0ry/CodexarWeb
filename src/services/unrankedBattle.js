...
async function initUnrankedBattle() {
    const token = localStorage.getItem('access_token');
    if (!token) { window.location.href = 'Login.html'; return; }

    const params = new URLSearchParams(window.location.search);
    const matchId = params.get('id');
    if (!matchId) { window.location.href = 'Unranked.html'; return; }

    try {
        const res = await fetch(`${API_BASE}/matchmaking/match/${matchId}`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        if (!res.ok) { window.location.href = 'Unranked.html'; return; }
        ...
        console.error(err);
        window.location.href = 'Unranked.html';
    }
...
function setupPlayers(data) {
    ...
    if (data.me.avatar) {
        document.getElementById('navAvatar').style.border = '1px solid var(--accent-green)';
    }
...
// STEP 1: Check — local test, no ELO change
...
// STEP 2: Save — submits win claim to server (unranked=true → no ELO)
async function handleSave() {
    ...
    try {
        ...
        const submitRes = await fetch(`${API_BASE}/matchmaking/match/${matchId}/submit`, {
            ...
            body: JSON.stringify({ results: mockResults, unranked: true })
        });
...
function endMatch(isWinner) {
    ...
    if (isWinner) {
        ...
        title.className = "win";
        desc.textContent = "¡Buen juego! Ningún ELO fue modificado.";
    } else {
        title.textContent = "¡PERDISTE!";
        title.className = "lose";
        document.getElementById('opProgressBar').style.width = '100%';
        desc.textContent = "Tu rival llegó primero. ¡Sigue practicando!";
    }
}
...
document.addEventListener('DOMContentLoaded', initUnrankedBattle);
